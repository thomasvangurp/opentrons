...
PROTOCOL_RUNNING_LOCK = LockFile('/tmp/resin/resin-updates')
...
def run():
    ...
    def _run_with_lock():
        if PROTOCOL_RUNNING_LOCK.is_locked():
            raise RuntimeError("Robot is running a protocol or updating") 
        else:
            with PROTOCOL_RUNNING_LOCK:
                _run()

    threading.Thread(target=_run_with_lock, args=()).start()
...
def aspirate_from_current_position():
    ...
    try:
        _, _, robot_max_z = app.robot._driver.get_dimensions()
        current_z = app.robot._driver.get_position()['current']['z']
